...
async function showStatus() {
    try {
        ...
        console.log(`Успешность: ${stats.successRate.toFixed(1)}%`);
        ...
        try {
            ...
            for (const [ticker, percent] of Object.entries(desired.portfolio)) {
                console.log(`${ticker}: ${percent.toFixed(2)}%`);
            }
        ...
        try {
            const timing = await marketTiming.findQuietestTradingTime(BALANCER_CONFIG.TINKOFF_ETFS[0]);
            console.log(`Самое спокойное время: ${timing.quietestHour}:00 UTC`);
            console.log(`Минимальная волатильность: ${timing.minVolatility.toFixed(4)}`);
...
async function runOnce() {
    try {
        ...
        if (result.success) {
            ...
            console.log(`✗ Балансировка не выполнена: ${result.reason || result.error}`);
        }
        return result;
    }
    catch (error) {
...
async function testConfig() {
    try {
        ...
        const timing = await marketTiming.findQuietestTradingTime(BALANCER_CONFIG.TINKOFF_ETFS[0].slice(0, 3));
        ...
        console.log(`✓ Получен портфель: ${portfolio.totalValue.toFixed(2)} RUB`);
...
async function main() {
    ...
    console.log(`Фондов для балансировки: ${BALANCER_CONFIG.TINKOFF_ETFS[0].length}`);
    switch (command) {
        case 'status':
            await showStatus();
            break;

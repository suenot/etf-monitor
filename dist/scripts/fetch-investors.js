...
async function fetchInvestorsData() {
    ...
    try {
        ...
        const investorsData = await scraper.getAllInvestorsData(figis[0]);
        ...
        for (const data of investorsData) {
            try {
                const result = await db.saveInvestorsSnapshot(data.figi, data.investors, data.capturedAt);

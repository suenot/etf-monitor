...
class BalancerService {
    ...
    async getInstrumentInfo(figi) {
        try {
            const api = await tinkoffApi.connect();
            const response = await api.instruments.getInstrumentBy({ idType: 'INSTRUMENT_ID_TYPE_FIGI', id: figi,
                classCode: "" });
    ...
    async rebalance() {
        try {
            ...
            const isGoodTime = await marketTiming.isGoodTimeToTrade(BALANCER_CONFIG.TINKOFF_ETFS[0]);
            if (!isGoodTime) {
    ...
    getStats() {
        return {
            ...
            config: {
                ...
                etfsCount: BALANCER_CONFIG.TINKOFF_ETFS[0].length,

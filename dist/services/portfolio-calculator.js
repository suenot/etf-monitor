...
class PortfolioCalculatorService {
    ...
    async getTinkoffEtfsData() {
        try {
            ...
            const tinkoffEtfs = allEtfs.filter(etf => BALANCER_CONFIG.TINKOFF_ETFS[0].includes(etf.ticker));
            console.log(`Найдено ${tinkoffEtfs.length} фондов Тинькофф из ${BALANCER_CONFIG.TINKOFF_ETFS[0].length} в конфигурации`);

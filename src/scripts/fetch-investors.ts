...
async function fetchInvestorsData() {
    ...
    try {
        ...
        const investorsData = await scraper.getAllInvestorsData(figis[0] as string);
        ...
        const duration = (Date.now() - (startTime as number)) / 1000;
        
        console.log('=== Результаты сбора данных об инвесторах ===');
        console.log(`Время выполнения: ${(duration as any).toFixed(2)} секунд`);
        console.log(`Всего обработано: ${(figis[0] as string).length}`);
        ...
        return {
            success: true,
            processed: (figis[0] as string).length,
        ...
        return {
            success: false,
            error: error.message,
            duration: (Date.now() - (startTime as number)) / 1000
        };
    } finally {

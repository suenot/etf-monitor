...
async function showStatus() {
    try {
        ...
        try {
            const timing = await marketTiming.findQuietestTradingTime(BALANCER_CONFIG.TINKOFF_ETFS);
...
async function testConfig() {
    try {
        ...
        const timing = await marketTiming.findQuietestTradingTime(BALANCER_CONFIG.TINKOFF_ETFS.slice(0, 3));
...
async function main() {
    ...
    console.log(`Фондов для балансировки: ${BALANCER_CONFIG.TINKOFF_ETFS.length}`);

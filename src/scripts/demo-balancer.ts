...
class DemoBalancer {
    ...
    calculateDesiredPortfolio() {
        ...
        for (const [ticker, data] of Object.entries(this.etfData)) {
            totalMarketCap += (data as any).marketCap;
            console.log(`${ticker} (${(data as any).name}): ${((data as any).marketCap / 1000000000).toFixed(1)} млрд RUB`);
        }
        ...
        for (const [ticker, data] of Object.entries(this.etfData)) {
            const proportion = ((data as any).marketCap / totalMarketCap) * 95; // 95% в фонды, 5% в рубли
            desiredPortfolio[ticker] = proportion;
            console.log(`${ticker}: ${(proportion as any).toFixed(2)}%`);
        }
    ...
    analyzeMarketTiming() {
        ...
        for (let hour = 0; hour < 24; hour++) {
            ...
            console.log(`${hour.toString().padStart(2, '0')}:00 - ${(vol as any).toFixed(4)}${marker}`);
        }
        
        console.log(`\nСамое спокойное время: ${quietestHour}:00 UTC (волатильность: ${(minVolatility as any).toFixed(4)})`);
        console.log(`Текущее время: ${currentHour}:00 UTC (волатильность: ${this.volatilityData[currentHour].toFixed(4)})`);
        
        const isGoodTime = Math.abs((currentHour as number) - (quietestHour as number as number)) <= 1;
    ...
    calculateRebalanceOperations(currentPortfolio,  desiredPortfolio) {
        ...
        for (const [ticker, desiredPercent] of Object.entries(desiredPortfolio.portfolio)) {
            ...
            const percentDiff = (desiredPercent as number) - (currentPercent as number);
            
            let action = 'НЕТ';
            if (Math.abs(percentDiff as number) > 2) { // Порог 2%
                const targetValue = ((desiredPercent as number) / 100) * totalValue;
                const currentValue = currentPosition ? currentPosition.value : 0;
                const valueDiff = targetValue - currentValue;
                
                if (Math.abs(valueDiff as number) > 10000) { // Минимум 10k рублей
                    action = valueDiff > 0 ? `КУПИТЬ ${Math.abs(valueDiff as number).toFixed(0)} RUB` : `ПРОДАТЬ ${Math.abs(valueDiff as number).toFixed(0)} RUB`;
                    ...
                    operations.push({
                        ...
                        desiredPercent,
                        percentDiff,
                        valueDiff: Math.abs(valueDiff as number),
                        priority: Math.abs(percentDiff as number)
                    });
                }
            }
            ...
            console.log(
                ticker.padEnd(8) + 
                `${(currentPercent as any).toFixed(1)}%`.padEnd(12) + 
                `${(desiredPercent as any).toFixed(1)}%`.padEnd(12) + 
                `${percentDiff > 0 ? '+' : ''}${(percentDiff as any).toFixed(1)}%`.padEnd(12) + 
                action
            );
        }
    ...
    async demonstrateBalancing() {
        ...
        if (operations.length > 0) {
            ...
            for (const [index, operation] of operations.entries()) {
                console.log(`${index + 1}. ${operation.action} ${operation.ticker}: ${(operation.valueDiff as any).toFixed(0)} RUB`);
                console.log(`   Изменение: ${(operation.currentPercent as any).toFixed(1)}% → ${(operation.desiredPercent as any).toFixed(1)}%`);

...
class MarketTimingService {
    private volatilityHistory: Map<string, any> = new Map();
    ...
    async getHistoricalData(figi: string,  days = 7) {
        try {
            ...
            const response = await api.marketdata.getCandles({
                ...
                from,
                to,
                interval: 'CANDLE_INTERVAL_HOUR' as any // Часовые свечи
            });
    ...
    calculateHourlyVolatility(candles: any[]) {
    ...
    async analyzeMarketVolatility(tickers: string) {
        try {
    ...
    async findQuietestTradingTime(tickers: string) {
        try {
    ...
    getTimeRecommendation(quietestHour,  minVolatility,  isQuietTime) {
        ...
        if (currentHour === quietestHour && isQuietTime) {
            return {
                action: 'trade',
                reason: `Текущее время (${currentHour}:00 UTC) является самым спокойным с волатильностью ${(minVolatility as any).toFixed(4)}`,
                confidence: 'high'
            };
        }
    ...
    async isGoodTimeToTrade(tickers: string) {
        try {

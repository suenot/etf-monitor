...
class BalancerService {
    ...
    async getInstrumentInfo(figi: string) {
        try {
            const api = await tinkoffApi.connect();
            const response = await api.instruments.getInstrumentBy({ idType: 'INSTRUMENT_ID_TYPE_FIGI' as any, id: figi
            , classCode: "" });
    ...
    async getCurrentPortfolio() {
        try {
            ...
            console.log(`Текущий портфель: ${(portfolio.totalValue as any).toFixed(2)} RUB, ${portfolio.positions.length} позиций`);
    ...
    calculateRebalanceOperations(currentPortfolio,  desiredPortfolio) {
        ...
        for (const [ticker, desiredPercent] of Object.entries(desiredPortfolio.portfolio)) {
            ...
            if (Math.abs(percentDiff as number) > BALANCER_CONFIG.MAX_DEVIATION_PERCENT) {
                ...
                if (Math.abs(valueDiff as number) > BALANCER_CONFIG.MIN_ORDER_VALUE) {
                    operations.push({
                        ticker,
                        figi: currentPosition?.figi,
                        action: valueDiff > 0 ? 'BUY' : 'SELL',
                        currentPercent: (currentPercent as any).toFixed(2),
                        desiredPercent: (desiredPercent as any).toFixed(2),
                        percentDiff: (percentDiff as any).toFixed(2),
                        currentValue: (currentValue as any).toFixed(2),
                        targetValue: (targetValue as any).toFixed(2),
                        valueDiff: Math.abs(valueDiff as number).toFixed(2),
                        priority: Math.abs(percentDiff as number) // Приоритет по размеру отклонения
                    });
                }
            }
        }
    ...
    async rebalance() {
        try {
            ...
            const isGoodTime = await marketTiming.isGoodTimeToTrade((BALANCER_CONFIG.TINKOFF_ETFS[0] as string));
            if (!isGoodTime) {
            ...
            for (const operation of operations) {
                ...
                if (result.success) {
                    ...
                    console.error(`✗ Ошибка операции ${operation.action} ${operation.ticker}: ${(result as any).error}`);
                }
    ...
    getStats() {
        return {
            ...
            config: {
                ...
                etfsCount: (BALANCER_CONFIG.TINKOFF_ETFS[0] as string).length,

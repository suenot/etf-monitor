...
class HealthCheck {
    ...
    async checkDataFreshness() {
        try {
            ...
            const etfAge = etfLastUpdate ? (now - new Date(etfLastUpdate)) / (1000 * 60) : null; // в минутах
            const investorsAge = investorsLastUpdate ? (now - new Date(investorsLastUpdate)) / (1000 * 60) : null;
